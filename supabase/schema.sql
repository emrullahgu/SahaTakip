...
-- Yetki yükseltme koruması: kullanıcı kendi profilinde role/approval_status/
-- approved_at/approved_by/rejection_reason sütunlarını DOĞRUDAN UPDATE ile
-- değiştiremez (self-promote engellenir). Bu sütunlar yalnız SECURITY DEFINER
-- RPC'leri (set_user_role / set_user_approval) üzerinden değişir; o fonksiyonlar
-- owner (postgres) olarak çalıştığı için current_user 'authenticated' değildir.
