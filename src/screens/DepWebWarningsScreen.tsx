// Faz 51 — DepWebWarningsScreen (POZ-DEV-218)
...
export default function DepWebWarningsScreen() {
  const [items, setItems] = useState<DepBuildError[]>([]);
  useFocusEffect(useCallback(() => { (async () => setItems(await listDepErrors()))(); }, []));

  const webItems = useMemo(() => items.filter(i => i.platform === 'web'), [items]);
  const stats = useMemo(() => ({
    total: webItems.length,
    open: webItems.filter(i => !i.fixed).length,
    error: webItems.filter(i => i.severity === 'error').length,
  }), [webItems]);
    ...
    <SafeAreaView style={s.safe} edges={['bottom']}>
      <ScrollView contentContainerStyle={s.scroll}>
        <View style={s.heroCard}>
          ...
          <Text style={s.heroT}>Web Platform Uyarıları</Text>
        ...
        {webItems.length === 0 && (
          <View style={s.empty}>
            ...
            <Text style={s.emptyT}>Web platformunda uyarı yok</Text>
          </View>
        )}
      </ScrollView>
    </SafeAreaView>
